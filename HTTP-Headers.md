...
- **Referrer-Policy**: Controls how much of the current URL is shared when a user clicks a link to another site. Without this, sensitive URL parameters (like password reset tokens) can leak to third parties.
- **Permissions-Policy**: Restricts which browser features (camera, microphone, geolocation) your page can use. This limits the damage if an attacker manages to inject malicious code.
